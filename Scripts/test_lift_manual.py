...
def tick(dt):
 ...
 def quit():
  (ROOT/'Documentation/LiftManualRuntime.json').write_text(json.dumps(report,indent=2));u.unregister_slate_post_tick_callback(handle);u.SystemLibrary.quit_game(w,pc,u.QuitPreference.QUIT,False)
 ...
 try:
  ...
  if phase==0 and t>7:
   ...
   u.SystemLibrary.execute_console_command(w,'HighResShot 1280x720 filename="'+(ROOT/'Documentation/LiftManualView.png').as_posix()+'"');advance()
